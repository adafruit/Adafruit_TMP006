...
boolean Adafruit_TMP006::begin(void) {
  Wire.begin();
...
uint16_t Adafruit_TMP006::read16(uint8_t a) {
  ...
  Wire.beginTransmission(TMP006_I2CADDR); // start transmission to device 
#if (ARDUINO >= 100)
  ...
  Wire.beginTransmission(TMP006_I2CADDR); // start transmission to device 
  Wire.requestFrom(TMP006_I2CADDR, 2);// send data n-bytes read
#if (ARDUINO >= 100)
